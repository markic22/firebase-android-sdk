...
public final class LocalStore {
  ...
  /**
   * Returns a promise of a boolean to indicate if the given bundle has already been loaded and the
   * create time is newer than the current loading bundle.
   */
  public boolean hasNewerBundle(BundleMetadata bundleMetadata) {
    @Nullable
    BundleMetadata existingMetadata =
        persistence.runTransaction(
            "Has newer bundle", () -> bundleCache.getBundleMetadata(bundleMetadata.getBundleId()));
   ...
   * <p>LocalDocuments are re-calculated if there are remaining mutations in the queue.
   */
  public void applyBundledDocuments(
  ...
  public void saveNamedQuery(NamedQuery namedQuery, ImmutableSortedSet<DocumentKey> documentKeys) {
    ...
    TargetData existingTargetData = allocateTarget(namedQuery.getQuery().toTarget());
  ...
  public @Nullable Query getNamedQuery(String queryName) {
    return persistence.runTransaction(
        "Get named query",
        () -> {
          NamedQuery namedQuery = bundleCache.getNamedQuery(queryName);
          return namedQuery != null ? namedQuery.getQuery() : null;
        });
  }
